...
\revb{The visual system, as shown in Figure~\ref{NeuralProcessing}a, comprises rods and cones that sense stimuli of different wavelengths. This system accumulates weighted logarithmic responses from input stimuli to represent chrominance (color) and luminance (brightness). For instance, brightness changes are detected when half the weights are positive and half are negative. The resulting signals feed the associative cortex for object linking and the occipital cortex for pattern processing. As shown in Figure~\ref{NeuralProcessing}b, the visual system extracts features such as edges and corners to detect objects} \cite{nixon2019feature}. \revb{Edge and corner detection enables humans to compare noisy and degraded images quickly and accurately. Hubel and Wiesel}~\cite{hubel1979brain} \revb{demonstrated that visual cells exhibit hierarchical responses: simpler cells track the general structure of scenes and feed results to cells that detect full texture complexity. Inspired by this phenomenon, vision processing extracts interest points from multiple pixels rather than processing pixel-by-pixel. This multi-pixel evaluation handles distortion and enhances image recovery.}
\begin{figure}[t]
	\centering
	\includegraphics[width=.45\textwidth]{NeuralProcessing.pdf}
	\caption{\revb{Neural vision processing: (a) structure of input to the human visual system; (b) sensitivity of the human visual system to edges}~\cite{nixon2019feature}.}
	\centering
	\label{NeuralProcessing}
\end{figure}

\revb{This section evaluates several feature extraction methods that detect stable interest points from scenes. An ideal interest point possesses: (1) a well-founded mathematical definition; (2) a clear, well-defined position in the image; (3) information-rich surrounding pixels; and (4) stability under perturbations and affine distortions}~\cite{bouris2010fast}.

\subsection{Feature Extraction Algorithms}
\revb{In feature extraction algorithms, descriptors extracted from image frames are compared with descriptors from labeled datasets to generate predictions. Higher descriptor counts and greater robustness to scale and orientation variations yield improved accuracy.}

\subsubsection{Harris Corner Detection (HCD)}
\revb{Harris et al. pioneered edge and corner detection using feature extraction methods}~\cite{harris1988combined}. \revb{Their Harris Corner Detection (HCD) algorithm extracts 3D object locations. By shifting a local window by small amounts, the algorithm considers three scenarios: (1) when the local window covers flat image patches, small intensity changes result; (2) a local window moving perpendicular to edges shows maximum changes while movement along edges shows small changes; (3) a window moving over corners shows large changes in every direction. The autocorrelation function in Equation~\ref{lof1} addresses all these scenarios. Here, $w_{u,v}$ is a circular weight window based on the Gaussian function to avoid noisy responses from rectangular equivalents. A small movement $(u,v)$ of the local window over pixel p(x,y) creates a curvature-like intensity change centered on this weight window. Equation~\ref{lof1} can be transformed into Equation~\ref{lof2}, where the symmetric matrix M(x,y) describes the window shape at the origin pixel. Extracting the eigenvalues $\alpha$ and $\beta$ of $M(x,y)$ makes this descriptor matrix rotationally invariant. When both $\alpha$ and $\beta$ exceed a specific threshold, they indicate a corner region; when only one eigenvalue exceeds the threshold, the window detects an edge region}~\cite{lam2017lowering}. \revb{As shown in Equation~\ref{lof3}, eigenvalue decomposition is avoided by calculating ratios using the trace ($Tr(M)$) and determinant ($Det(M)$). The parameter $k$ is an empirical value ranging from 0.4 to 0.6. Corner regions have positive $R$ values, while negative $R$ values represent edge regions. In the non-maximal suppression stage, $R$ is compared to a threshold value $T$, and non-corner $R$ values below $T$ are set to 0 to minimize storage requirements.}
